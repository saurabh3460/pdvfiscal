...
func NotepadReadGET(w http.ResponseWriter, r *http.Request) {
	...
	notes, err := m.NotesByUserID(userID)
	if err != nil {
		log.Println(err)
		notes = []m.Note{}
	}
...
func NotepadCreatePOST(w http.ResponseWriter, r *http.Request) {
	...
	// Get database result
	err := m.NoteCreate(content, userID)
...
func NotepadUpdateGET(w http.ResponseWriter, r *http.Request) {
	...
	// Get the note
	note, err := m.NoteByID(userID, noteID)
...
func NotepadUpdatePOST(w http.ResponseWriter, r *http.Request) {
	...
	// Get database result
	err := m.NoteUpdate(content, userID, noteID)
	// Will only error if there is a problem with the query
	if err != nil {
		log.Println(err)
		sess.AddFlash(view.Flash{"An error occurred on the server. Please try again later.", view.FlashError})
...
func NotepadDeleteGET(w http.ResponseWriter, r *http.Request) {
	...
	// Get database result
	err := m.NoteDelete(userID, noteID)
	// Will only error if there is a problem with the query
	if err != nil {
		log.Println(err)
		sess.AddFlash(view.Flash{"An error occurred on the server. Please try again later.", view.FlashError})
