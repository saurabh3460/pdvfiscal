...
func ReadLoggingConfig(modes []string, logsPath string, cfg *ini.File) error {
	...
	for _, mode := range modes {
		...
		if err != nil {
			...
			return errors.Wrapf(err, "failed to get config section log.%s", mode)
		}
		...
		case "file":
			...
			fileName := sec.Key("file_name").MustString(filepath.Join(logsPath, "log.log"))
			...
			if err := os.MkdirAll(dpath, os.ModePerm); err != nil {
				...
				return errors.Wrapf(err, "failed to create log directory %q", dpath)
			}
			...
			if err := fileHandler.Init(); err != nil {
				...
				return errors.Wrap(err, "failed to initialize file handler")
			}
